import { useEffect, useState } from "react";

import MetricCard from "../components/MetricCard";
import DataTable from "../components/DataTable";

import { buscarTabela } from "../services/api";

function Planejamento() {
    const [erro, setErro] = useState(null);
    const [planejamento, setPlanejamento] = useState([]);
    const [carregando, setCarregando] = useState(true);
    const [total, setTotal] = useState(0);
    const [metricas, setMetricas] = useState(null);

    useEffect(() => {
        async function carregarPlanejamento() {
            try {
                setCarregando(true);
                setErro(null);

                const resultado = await buscarTabela('planejamentos');

                setPlanejamento(resultado.data);
                setTotal(resultado.total);
                setMetricas(resultado.metricas);
            } catch (error) {
                console.error(
                    'Erro ao carregar planejamento:',
                    error
                );

                setErro(
                    'Não foi possível carregar o planejamento'
                );
            } finally {
                setCarregando(false);
            }
        }

        carregarPlanejamento();
    }, []);

    const columns = [
        { key: 'id', label: 'id' },
        { key: 'dataSolicitacao', label: 'dataSolicitacao' },
        { key: 'descricao', label: 'descricao' },
        { key: 'tipo', label: 'tipo' },
        { key: 'status', label: 'status' },
        { key: 'responsavelId', label: 'responsavelId' },
        { key: 'urgencia', label: 'urgencia' },
        { key: 'parecerDoResponsavel', label: 'parecerDoResponsavel' },
        { key: 'prazoDeConclusao', label: 'prazoDeConclusao' },
        { key: 'clienteId', label: 'clienteId' },
        { key: 'parecerDoCliente', label: 'parecerDoCliente' },
        { key: 'assinaturaDoCliente', label: 'assinaturaDoCliente' },
        { key: 'dataDeConclusao', label: 'dataDeConclusao' },
        { key: 'solicitanteId', label: 'solicitanteId' },
        { key: 'dataInicio', label: 'dataInicio' },
        { key: 'privacidade', label: 'privacidade' },
        { key: 'foto', label: 'foto' }
    ];

    if (carregando) {
        return (
            <div>
                <h2>Planejamento</h2>
                <p>Carregando dados...</p>
            </div>
        );
    }

    if (erro) {
        return (
            <div>
                <h2>Planejamento</h2>
                <p>{erro}</p>
            </div>
        );
    }

    return (
        <section className="page">
            <header className="page-header">
                <div>
                    <h2>Planejamento</h2>

                    <p>
                        Visualização e análise da tabela
                        de planejamento migrada.
                    </p>
                </div>
            </header>

            <div className="metrics-grid">
                <MetricCard
                    title="Registros"
                    value={total.toLocaleString('pt-BR')}
                />

                <MetricCard
                    title="Tamanho da resposta"
                    value={`${metricas.tamanhoMB.toFixed(2)} MB`}
                />

                <MetricCard
                    title="Tempo de carregamento"
                    value={`${metricas.tempoMs.toFixed(0)} ms`}
                />
            </div>

            <DataTable
                columns={columns}
                data={planejamento}
            />
        </section>
    );
}

export default Planejamento;